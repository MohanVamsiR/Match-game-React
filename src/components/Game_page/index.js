...
import TabsListItems from '../TabItems'
...
class GamePage extends Component {
  state = {
    ...
    tabId: 'FRUIT',
    isGameOn: true,
    questionImg: imagesList[0].imageUrl,
    clickedImagesList: [],
  ...
  tick = () => {
    const {timer, isGameOn} = this.state
    if (timer > 0 && isGameOn) {
  ...
  filteredList = () => {
    ...
    const results = imagesList.filter(each => each.category === tabId)
  ...
  onChangeImage = id => {
    const {imageId, clickedImagesList, isGameOn} = this.state
    if (imageId === id && isGameOn) {
      ...
      this.setState({
        questionImg: imagesList[`${randomImage}`].imageUrl,
        imageId: imagesList[`${randomImage}`].id,
      })
    } else {
      this.setState({isGameOn: false, timer: 0})
    }
  }
  ...
  render() {
    const {timer, isGameOn, clickedImagesList, questionImg} = this.state
    ...
    return (
      <div className="game-container">
        <nav className="nav-bar">
          ...
          />
          <ul className="live-score">
            <li className="lis">
              <p className="score">score:</p>
              <p className="spanEl">{totalScore}</p>
            </li>
            <li className="lis-ti">
              <img
        ...
        {isGameOn ? (
          <div className="game-cont">
            <img src={questionImg} alt="match" className="display-image" />
            <ul className="ul-tab">
              {tabsList.map(each => (
                <TabsListItems
          ...
          <div className="score-card">
            <ScoreCard score={totalScore} playAgain={this.playAgain} />
          </div>
        )}
      </div>
    )
